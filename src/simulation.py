...
class SimulationInput:
    ...
    def __init__(self, a, e, periapsis, long, i, m0, spr_k, spr_l0, spr_l1, spr_l2,
        ...
        self.semi_major = a
        """ Большая полуось начальной орбиты (метры) """
        self.eccentricity = e
        """ Эксцентриситет начальной орбиты (б/р) """
        self.arg_periapsis = periapsis
        """ Аргумент перицентра начальной орбиты (радианы) """
        self.long_ascend = long
        """ Долгота восходящего узла начальной орбиты (радианы) """
        self.inclination = i
        """ Угол наклонения начальной орбиты (радианы) """
        self.mean_anomaly = m0
        """ Средняя аномалия начальной орбиты (радианы) """
        self.spring_stiffness = spr_k
        """ Коэффициент жесткости пружины толкателя (Н/м) """
        self.spring_l0 = spr_l0
        """ Длина пружины толкателя в недеформированном состоянии (метры) """
        self.spring_l1 = spr_l1
        """ Конечная длина пружины толкателя (упор) (метры) """
        self.spring_l2 = spr_l2
        """ Начальная длина пружины толкателя (метры) """
        self.start_yaw = yaw
        """ Угол "рысканья" направления действия силы толкателя, отн. начального вектора скорости РН (радианы) """
        self.start_pitch = pitch
        """ Угол "тангажа" направления действия силы толкателя, отн. начального вектора скорости РН (радианы) """
        self.mass_lv = mass_lv
        """ Масса РН (кг) """
        self.mass_sc = mass_sc
        """ Масса КА (кг) """
        self.sim_time = sim_time
        """ Продолжительность моделирования (секунды) """
        self.print_frequency = print_freq
        """ Частота печати результатов моделирования в csv-таблицу """
...
def process_result(sim_out: SimulationOutput, filename_csv: str):
    """
    Печатает результаты моделирования динамики отделения КА от ступени РН в виде:
        - csv-файла,
