...
def kepler2eci(a: float, e: float, i: float, long: float, periapsis: float, m0: float) -> np.ndarray:
    ...
    :return: Матрица 2х3, в первой строке проекции радиус-вектора в метрах,
             во второй – вектора скорости материальной точки в м/сек.
...
def get_state_relative(src_state: np.ndarray, distance: float, yaw: float, pitch: float) -> tuple:
    """
    Рассчитывает радиус-вектор точки, относительно заданной
    по известному расстоянию между ними и ориентации искомой точки
    ...
    :param src_state: Матрица 2х3, в первой строке проекции радиус-вектора в метрах,
                      во второй – вектора скорости исходной точки в м/сек.
    ...
    :return: Кортеж, первый элемент которого единичный вектор-слагаемое, а второй – матрица 2х3,
             в первой строке проекции радиус-вектора в метрах, во второй – вектора скорости искомой точки в м/сек.
    """
...
def motion_equation_rhs(t: float, y_vecs: np.ndarray, *constants) -> np.ndarray:
    ...
    :param constants: Постоянные величины, необходимые для решения ДУ: параметры пружины
                      (жесткость в Н/м, конечная длина, длина в недеформированном состоянии в метрах),
                      масса РН и КА в кг, единичный вектор ``e_lv2sc``, задающий
                      направление действия силы упругости пружинного толкателя
