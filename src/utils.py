...
def show_anim(figure: Figure, func_draw: Callable, frame_freq: float, t: np.ndarray, *args):
    ...
    Параметр ``frame_freq`` задает количество кадров, выводящихся на секунду
    времени, используя в качестве шкалы массив моментов времени ``t``.
    `Таким образом, частота` ``frame_freq`` `не может быть выше частоты печати
    временной шкалы` ``t`` `.`
    Параметр args задает варьируемый список параметров функции отрисовки кадра.
